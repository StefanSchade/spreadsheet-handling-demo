...
* `apply_fks` materializes helper columns
* `flatten_headers` keeps unpacked JSON readable on the way back
...
== What Comes Back On Unpack

After the workbook is unpacked again, the helper columns stay understandable because
the pipeline flattens headers before export.
